...
class powerfoxIO extends IPSModule
{
    // powerfox API
    private const POWERFOX_BASE_URL = 'https://backend.powerfox.energy';
    ...
    private const HISTORY_DEVICE_CSV = '/reportcsv';
    private const OPERATING = '/operating';
    private const OPERATING_CSV = '/reportcsv';
    ...
    public function ForwardData($JSONString)
    {
        ...
        if($method == 'GetCurrentData')
        {
            return $this->GetCurrentData($device_id);
        }
        ...
        {
            return $this->GetHistoryDevices();
        }
        if($method == 'GetHistoryDevice')
        {
            return $this->GetHistoryDevice($device_id);
        }
        if($method == 'GetHistoryDeviceCSV')
        {
            return $this->GetHistoryDeviceCSV($device_id);
        }
        if($method == 'GetPowerHistory')
        {
            return $this->GetPowerHistory($device_id);
        }
        if($method == 'GetPowerHistoryCSV')
        {
            return $this->GetPowerHistoryCSV($device_id);
        }
    ...
    {
        $this->SendDataToChildren(json_encode(['DataID' => '{797F8101-BBE4-A81E-BDBC-1A81101E4732}', 'Buffer' => $Text]));
    }
    ...
    {
        $url = self::POWERFOX_BASE_URL . self::API_20 . self::ALL_DEVICES;
        $devices = $this->SendData($url);
        return $devices;
    }

    private function GetHistoryDevice($device_id)
    {
    ...
    public function GetHistoryDevices()
    {
    ...
    private function GetHistoryDeviceCSV($device_id)
    {
        $url = self::POWERFOX_BASE_URL . self::API_20 . self::MY_DEVICE . $device_id . self::HISTORY_DEVICE_CSV;
        $devices = $this->SendData($url);
        return $devices;
    }
     ...
     * @return bool|string
     */
    private function GetPowerHistory($device_id)
    {
        $url = self::POWERFOX_BASE_URL . self::API_20 . self::MY_DEVICE . $device_id . self::OPERATING;
        $devices = $this->SendData($url);
        return $devices;
    }
     ...
     * @return bool|string
     */
    private function GetPowerHistoryCSV($device_id)
    {
        $url = self::POWERFOX_BASE_URL . self::API_20 . self::MY_DEVICE . $device_id . self::OPERATING_CSV;
        $devices = $this->SendData($url);
        return $devices;
    }
    ...
    protected function FormStatus()
    {
        $form = [
            ...
            [
                'code' => 204,
                'icon' => 'error',
                'caption' => 'no password selected.'
            ],
            [
                'code' => 205,
                'icon' => 'error',
                'caption' => 'no user or password selected.'
            ]
        ];
