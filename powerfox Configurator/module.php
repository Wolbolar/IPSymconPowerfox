...
class powerfoxConfigurator extends IPSModule
{
    ...
    private function GetDivision($meter_type)
    {
        $meter_type = strval($meter_type);
        $description = '';
        $types = [
            0 => ['value' => self::ELECTRICITY_METER, 'description' => $this->Translate('Electricity meter')],
            1 => ['value' => self::COLD_WATER_METER, 'description' => $this->Translate('Cold water meter')],
            2 => ['value' => self::HOT_WATER_METER, 'description' => $this->Translate('Hot water meter')],
            3 => ['value' => self::HEAT_METER, 'description' => $this->Translate('Heat meter')],
            4 => ['value' => self::GAS_METER, 'description' => $this->Translate('Gas meter')],
            5 => ['value' => self::COLD_AND_HOT_WATER_METER, 'description' => $this->Translate('Cold and hot water meters')],
            6 => ['value' => self::NO_TYPE_STORED, 'description' => $this->Translate('No type stored')]

        ];
        foreach($types as $type)
        {
            if($type['value'] == $meter_type)
            {
    ...
    private function GetDate($AccountAssociatedSince)
    {
        $date = date('d.m.Y H:i:s', $AccountAssociatedSince);
        return $date;
    }

    private function GetMainDevice($MainDevice)
    {
        if($MainDevice)
        {
            $text = $this->Translate('Main Device');
        }
        else{
            $text = $this->Translate('Device');
        }
        return $text;
    }
    ...
    private function RequestDataFromParent(string $endpoint)
    {
        ...
        $this->SendDebug('powerfox Request Response', $endpoint . ": " . $data, 0);
        return $data;
    }
    ...
    protected function FormHead()
    {
        ...
        $form = [
            ...
            [
                'type' => 'Label',
                'caption' => 'category for powerfox devices'
            ],
            [
                'name' => 'ImportCategoryID',
                'type' => 'SelectCategory',
                'caption' => 'category powerfox devices'
            ],
            [
                ...
                'columns' => [
                    ...
                    [
                        'name' => 'AccountAssociatedSince',
                        'caption' => 'Account Associated Since',
                        'width' => '200px'
                    ],
                    [
                        'name' => 'MainDevice',
    ...
    protected function FormActions()
    {
        ...
        $form = [
            ...
            [
                'type' => 'Label',
                'visible' => true,
                'caption' => 'Read powerfox configuration:'
            ],
            [
                'type' => 'Button',
                'visible' => true,
